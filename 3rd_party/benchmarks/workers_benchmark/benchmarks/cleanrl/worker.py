"""CleanRL worker benchmark -- PPO through MOSAIC's CleanRLWorkerRuntime.

Measures the overhead of:
  - subprocess spawn (CleanRLWorkerRuntime runs training as a child process)
  - sitecustomize.py import hooks (gym.make override, wandb patching, etc.)
  - log capture (stdout/stderr redirection)
  - cleanrl_worker.launcher module dispatch

Fastlane and gRPC are disabled so we isolate pure worker wrapping cost.
"""
...
def run_worker_benchmark(config) -> BenchmarkResult:
    """Run CleanRL PPO via the MOSAIC worker subprocess (no fastlane)."""
    ...
    worker_config = CleanRLWorkerConfig(
        run_id=f"bench_worker_{config.seed}",
        algo="ppo",
    ...
    # Disable fastlane explicitly
    env_overrides = {
        "GYM_GUI_FASTLANE_ONLY": "0",
    }

    elapsed, peak_mb, stdout, _ = run_subprocess_timed(cmd, env=env_overrides)

    # Parse final episodic return from CleanRL stdout
...
def _parse_last_return(stdout: str) -> float:
    """Extract the last episodic_return from CleanRL's stdout."""
    last = 0.0
